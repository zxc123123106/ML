...
*   模型在測試集 11 筆資料上，實際死亡數全部是 0，但模型有時預測出 0-3.54 隻死亡風險，所以目前測到的是「誤報傾向」，還不能評估它抓大量死亡事件的能力。

### 指標解釋
*   `MAE` = 1.6002：平均每筆約多估或少估 1.6 隻。因為實際值全是 0，所以這裡等同平均「多報」1.6 隻。
*   `RMSE` = 2.0616：比 MAE 更重罰大錯誤。最高幾筆預測是 3.54、3.31、2.90，但實際都是 0，因此 RMSE 被拉高。
*   `R²` = 0.0：這次不太有解釋價值，因為測試集實際 death_count 全部都是 0，沒有變異量可解釋。這不是代表模型完全沒用，而是這個測試切分無法檢驗「死亡事件」預測能力。
*   `n_samples` = 11：只有 11 筆有標籤測試資料，樣本很少，結論要保守。

### 預測明細
*   reports/test_predictions.csv 裡每列是：
*   `actual`：真實死亡數
*   `predicted`：模型預測死亡數，已限制不低於 0
*   `residual` = `actual` - `predicted`\
*   目前所有 actual = 0。殘差全部小於或等於 0，表示模型沒有低估，只存在不同程度的高估。

### 圖表說明
*   `prediction_vs_actual.png`：\
橫軸是真實死亡數，縱軸是預測死亡數，紅色虛線是完美預測。現在所有點都在 actual = 0 的垂直線上；越靠近原點越好，越往上代表誤報越大。

*   `residuals.png`：\
橫軸是預測值，縱軸是 actual - predicted。點都在 0 以下，代表模型傾向高估死亡數；最差約高估 3.54 隻。

*   `learning_curve.png`：\
顯示 XGBoost boosting 過程中的 RMSE。兩條線都下降，代表訓練過程有收斂。因樣本很少，不能只靠這張圖判定泛化能力，但目前沒有看到驗證曲線明顯爆升的典型過擬合形狀。

*   `feature_importance.png`：\
這是 XGBoost gain-based importance，代表模型分裂節點時最常、最有效使用哪些特徵。前幾名是 hardness_mean、temp_shallow_mean、temp_deep_min、mq137_6hr_change_max。它說的是「模型依賴程度」，不直接表示正負方向。

*   `shap_summary.png`：\
這是 SHAP 平均絕對貢獻度，代表每個特徵實際把預測值推高或拉低的影響大小。最重要的是 mq137_6hr_change_max，其次是 temp_deep_min、ph_mean、vertical_temp_diff_max。這比 feature importance 更接近「對這批預測的實際影響」。

*   `top_feature_shap_dependence.png`：\
這張看最重要特徵 mq137_6hr_change_max 和 SHAP 貢獻的關係。這次結果顯示某些較低區間的 6 小時氨氣變化會推高死亡預測，而更高值反而在部分樣本上拉低預測，表示模型學到的是非線性規則。但樣本只有 11 筆，這個「閾值」只能當提示，不能當穩定生物學結論。

### 目前最重要的解讀
模型現在偏保守或偏敏感：在沒有死亡的測試日，也會報出死亡風險。這在預警系統中不一定完全壞，因為誤報比漏報可能更可接受；但若要實務部署，需要更多包含死亡事件的測試資料，才能判斷模型是否真的能抓到高風險日。
